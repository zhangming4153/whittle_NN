...
def pick_gamma0_by_grid(events, T, a=1.5, mu0=0.5, eta0=None,
    ...
    if eta0 is None:
        eta0 = max((1.0 - mu0) * (n / T), 1e-3)

    best_g = None
    best_nll = np.inf

    for g in gamma_grid:
        ll = loglik_power_hawkes_full(events_s, obs, pre, R, eta0, mu0, g, a=a, T=T)
        nll = 1e50 if not np.isfinite(ll) else -ll
        if nll < best_nll:
            best_nll = nll
            best_g = g

    return eta0, mu0, best_g

def fit_power_hawkes_lbfgsb(events, T, a=1.5, theta0=None, maxiter=60, ftol=1e-4):
    events = np.asarray(events, float)
    events_s, obs, pre, R = precompute_full(events, T)
    n = obs.size
...
def fit_power_hawkes_A(events, T, a=1.5, maxiter=60, ftol=1e-4,
    ...
    eta0, mu0, gamma0 = pick_gamma0_by_grid(events, T, a=a, mu0=mu0, gamma_grid=gamma_grid)
    ...
    theta0 = np.array([eta0, mu0, gamma0], float)
    res = fit_power_hawkes_lbfgsb(events, T, a=a, theta0=theta0, maxiter=maxiter, ftol=ftol)
    return res
...
def estimate_power_hawkes(events, 
    ...
    eta0, mu0, gamma0 = pick_gamma0_by_grid(
        events, T, a=a, mu0=mu0, eta0=eta0, gamma_grid=refine_grid
    )
    theta0 = np.array([eta0, mu0, gamma0], float)
    res = fit_power_hawkes_lbfgsb(events, T, a=a, theta0=theta0, maxiter=maxiter, ftol=ftol)
